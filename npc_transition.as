...
For install and usage instructions, visit: https://github.com/Outerbeast/Npc-Transition/blob/main/README.md
...
bool blNpcTransitionRegistered = EntityRegister();
...
bool EntityRegister(bool blDeletePrevSaveSetting = true)
{
    ...
    CBaseTransition().LoadNpcs( g_Engine.mapname, blDeletePrevSaveSetting );
...
class CBaseTransition : ScriptBaseEntity
{
    ...
    TransitionType GetTransitionType(EHandle hMonster)
    {
        if( !hMonster )
            return UNDEFINED;
        ...
        return TransitionType( kvMonster.GetKeyvalue( "$i_transition_type" ).GetInteger() );
    }
    // This method is WIP
    ...
    bool CheckBBoxValid(Vector vecAbsMin, Vector vecAbsMax)
    {
        ...
        {
            bool 
                blXisValid = vecAbsMax.x > vecAbsMin.x,
                blYisValid = vecAbsMax.y > vecAbsMin.y,
                blZisValid = vecAbsMax.z > vecAbsMin.z;
    ...
    bool LoadNpcs(string strLoadMap = string( g_Engine.mapname ), bool blDeletePrevSave = true)
    {
        ...
        if( g_FileSystem.OpenFile( "scripts/maps/" + strNpcFile, OpenFile::READ ) is null )
            ...
            g_FileSystem.OpenFile( "scripts/maps/" + strNpcFile, OpenFile::READ ).Close();
...
final class CTransitionEntity : CBaseTransition
{
    ...
    {// If the entity has a brush model, treat it like a brush entity
        if( self.GetClassname() == "func_transition" && string( self.pev.model ).StartsWith( "*" ) )
        {
    ...
    dictionary@ NpcData(EHandle hMonster)
    {
        ...
        dictionary dictMonster =
        {
            ...
            { "origin",             ( pMonster.pev.origin + vecLandmark - self.pev.origin + vecGroundOffset ).ToString().Replace( ",", "" ) },
    ...
    array<dictionary>@ GetNpcs()
    {
        ...
        int iNumMonsters =  CheckBBoxValid( vecZoneCornerMin, vecZoneCornerMax ) ?
                            ...
                            g_EntityFuncs.MonstersInSphere( @P_ENTITIES, self.pev.origin, flZoneRadius );
    ...
    {   // Make sure to only save when the number of monsters in the zone matches or exceeds the minimum required
        const uint iNpcsInZone = CheckBBoxValid( vecZoneCornerMin, vecZoneCornerMax ) ?
                                ...
                                g_EntityFuncs.MonstersInSphere( array<CBaseEntity@>( iMinRequired ), self.pev.origin, flZoneRadius ); // !-LIMITATION-!: MonstersInSphere counts player instances. This results in undefined behaviour. Need a flagmask for this method!
    ...
    HookReturnCode Transition(const string& in strLevel)
    {   // Current level is restarting, for whatever reason.
        if( strLevel == g_Engine.mapname )
            return HOOK_CONTINUE;
        // There is a target map and we are not going there yet.
        if( strNextMap != "" && strLevel != strNextMap )
            return HOOK_CONTINUE;
...
final class CLandmarkEntity : CBaseTransition
{
    ...
    void PositionTransitionNpcs()
    {
        ...
        if( g_EntityFuncs.Instance( 0 ).FindMonstersInWorld( @P_ENTITIES, FL_MONSTER ) < 1 )
        {
            iNumPositioned = -1;
        ...
        for( uint i = 0; i < P_ENTITIES.length(); i++ )
        {
            ...
            g_EntityFuncs.SetOrigin( P_ENTITIES[i], P_ENTITIES[i].pev.origin + self.pev.origin );

            if( self.pev.angles != g_vecZero )
                P_ENTITIES[i].pev.angles = self.pev.angles;

            if( EntityIsStuck( P_ENTITIES[i] ) )
                g_EntityFuncs.SetOrigin( P_ENTITIES[i], self.pev.origin + vecGroundOffset + Vector( Math.RandomFloat( 0, 16 ), Math.RandomFloat( 0, 16 ), 0 ) );
